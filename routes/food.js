...
router.get('/make-all-printable', async (req, res) => {
  try {
    ...
    const result = await Food.updateMany({ systemId: user.systemId }, { printable: true });
...
router.get("/", async (req, res) => {
  ...
  const food = await Food.find({ deleted: false, unlimit: false, systemId: user.systemId });
  let totalCost = 0;
  food.forEach((item) => {
...
router.get("/getall", async (req, res) => {
  ...
  const food = await Food.find({ deleted: false, active: true, systemId: user.systemId });
  console.log(food)
  res.json(food);
});
...
router.get("/categories", async (req, res) => {
  try {
    ...
    const category = await Category.find({ systemId: user.systemId }).populate({
      path: "foods",
      match: { active: true, systemId: user.systemId },
    });
    const food = await Food.find({ deleted: false, unlimit: false, systemId: user.systemId });
...
router.get("/getall", async (req, res) => {
  try {
    ...
    const foods = await Food.find({ deleted: false, systemId: user.systemId })
      .sort({ name: 1 })
      .populate({
        path: "category",
        match: { systemId: user.systemId },
      }
      );
    res.json(foods);
  } catch (error) {
