 ...
 */
async function groupFoodsByCategory(foodIds,systemId) {
    try {
        ...
        const devices = await Devices.find({systemId:systemId})
        ...
        const foods = await Food.find({ _id: { $in: validFoodIds },systemId:systemId })
            .populate("category")
            .exec();
