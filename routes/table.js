...
router.get("/getall", async (req, res) => {
  ...
  const tables = await Table.find({ systemId: user.systemId });
...
router.get("/getByType/:type", async (req, res) => {
  ...
  if (type === "Families") {
    const tables = await Table.find({ Families: true, systemId: user.systemId });
    const systemSetting = await SystemSetting.findOne({ specialId: user.systemId })
    res.json({ tables, systemSetting });
  } else {
    const tables = await Table.find({ $or: [{ Families: false, systemId: user.systemId }, { systemId: user.systemId, Families: { $exists: false } }] }); 
    const systemSetting = await SystemSetting.findOne({specialId: user.systemId})
    res.json({ tables, systemSetting });
